...
exports.multipartUpload = function (options) {
  ...
  chunkedStream.on('data', function (data) {
    ...
    if(options && options.contentType.includes("image/")){
      
      console.log("entered to autorotate image");

      jo.rotate(data, {quality: 50}, (error, buffer, orientation, dimensions, quality) => {
        if (error) {
